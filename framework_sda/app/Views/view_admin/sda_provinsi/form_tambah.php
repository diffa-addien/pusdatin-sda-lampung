...
<div class="card">
  <div class="card-body">
    ...
    <form class="was-validated" action="<?=base_url('Admin/tambah_sda_prov')?>" method="POST"
      enctype="multipart/form-data">
      <div class="mb-3 row">
        <div class="col-8">
          <label for="judul_data" class="col-form-label">Nama Data</label>
          <div class="">
            <input type="text" class="form-control" name="judul_data" id="judul_data" value="<?= old('judul_data') ?>" required>
          </div>
        </div>
        <div class="col-4">
          ...
          <div class="">
            <input type="number" class="form-control" name="tahun" id="tahun" min="2000" max="2999"
              value="<?= old('tahun') == "" ? date('Y') : old('tahun')?>" placeholder="Tahun" required>
          </div>
        </div>
      </div>
      <div class="mb-3 row">
        ...
        <div class="col-sm-10">
          <select name="id_kategori" id="id_kategori" class="form-control" required>
            ...
            <option value="<?=$kat["id"]?>" <?= $kat["id"] == old("id_kategori") ? "selected" : ""?>><?=$kat["nama"]?></option>
            <?php endforeach ?>
          </select>
        </div>
      </div>
      <div class="mb-3 row">
        ...
        <textarea name="isi" class="form-control mx-2" rows="5" required><?= old('isi')?>
        </textarea>
      </div>
      <div class="mb-3 row">
        <div class="col-sm-6">
        ...
        <div class="col-sm-6">
          ...
          <input type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.zip" name="upload_dokumen" class="form-control d-none" id="upload_dokumen"
            title="Masukan nilai yang valid">
          <div class="mb-3">
            <span>Ket. File: </span>
...
$("#geojson_petak").change(function () {
  ...
  if(extension == "kml"){
    ...
    console.log(toGeoJSON.kml((new DOMParser()).parseFromString(filename, 'text/xml')));
  }
});
